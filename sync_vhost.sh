...
for project in "${projects[@]}"; do
    ...
    document_root="${project_info[2]}"
    project_path="${project_info[4]}"
    ...
    docker exec apache_php82-webserver sed -i "s/__document_root__/${document_root}/g" "/etc/apache2/sites-available/${project_name}.conf"
    docker exec apache_php82-webserver sed -i "s/__domain__/${project_name}/g" "/etc/apache2/sites-available/${project_name}.conf"
    # Activer le site
    ...
    # Compléter le fichier .conf
    docker exec apache_php82-webserver sed -i "s/__document_root__/${document_root}/g" "/etc/apache2/sites-available/${project_name}-ssl.conf"
